...
2. Branch strategy (created automatically at fresh-build start):
   `main` = everything (OS + site), the working branch; `staging` =
   preview - push work-in-progress for client review, QA-gate EXEMPT,
   never connected to hosting; `deploy` = GENERATED branch holding only
   the contents of site/ at its root - hosting pulls it. Deploy =
   `bash scripts/deploy-split.sh` - it checks a clean tree,
   ...
   and force-pushes the split to deploy. The pre-deploy hook also
...
- Deploying from an uncommitted working tree; committing manually to
  the deploy branch - it is generated output, and the deploy script's
  force-push is the only writer; connecting hosting to staging;
